...
from src.processing import get_cropped_image
from src.models import get_effnet
...
def predict_one_stage(img: np.ndarray, preprocess: bool = True, st_dict_path: str = None, data_path: str = None,
    ...
    """
    Predicts class: 'Ne vus', 'SebK', 'DF', 'BCC', or 'Melanoma' based on prediction of one model
    :param img: BGR image
...
def predict(img: np.ndarray, preprocess: bool = True, st_dict_path: str = None) -> str:
    """
...
if __name__ == "__main__":
    ...
    state_dict_path = r'..\data\demonstration\one_stage\final.pt'
    ...
    for i, image in enumerate(csv_images, 1):
        ...
        result = predict_one_stage(image, True, st_dict_path=state_dict_path, data_path=data_path)
        print(i, result)
        results.append(result)
    csv['res'] = results
    csv.to_csv(os.path.join(data_path, 'one_stage.csv'), index=False)
